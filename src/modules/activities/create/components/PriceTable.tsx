...
const PriceTable: React.FC<PriceTableProps> = ({ prices, level, priceDefinition, pricePackage }) => {
	...
	const handleRemovePrice = (price: Price | undefined) => {
		if (!price) return;
		console.log('remove currentActivity?.prices', currentActivity?.prices)
		console.log('remove pricesxxxx', prices)
		let pricesWithoutPrice = []
		if (price.id === 0) {
			pricesWithoutPrice = currentActivity?.prices?.filter(p => p.idx !== price.idx) ?? [];
		} else {
			pricesWithoutPrice = currentActivity?.prices?.filter(p => p.id !== price.id) ?? [];
		}
		
		if (price.saved === DBStatus.NEW) {
			console.log("prices after deleted NEW", pricesWithoutPrice)
			dispatch({
			...
			console.log("prices after deleted", [...pricesWithoutPrice, { ...price, saved: DBStatus.TOBEDELETED }])
			dispatch({
				type: ActivityActions.SetActivityForm,
				payload: {
					currentActivity: {
	...
	console.log('init currentActivity?.prices', currentActivity?.prices)
	console.log('init pricesxxxx', prices)
